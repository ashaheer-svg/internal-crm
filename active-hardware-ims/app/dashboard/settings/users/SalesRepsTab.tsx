                ...
                <table className="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                        {loading ? (
                            ...
                            <tr><td colSpan={5} className="py-10 text-center text-sm text-gray-500">No sales representatives found</td></tr>
                        ) : (
                                ...
                                <tr key={rep.id} className={rep.isActive ? "" : "bg-gray-50"}>
                                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">
                                        <div className="flex items-center">
                                            ...
                                            <span className={rep.isActive ? "" : "text-gray-500 italic"}>{rep.name}</span>
                                        </div>
                                    </td>
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                        <div className="flex flex-col space-y-1">
                                            {rep.email && (
                                                <div className="flex items-center"><Mail className="mr-1.5 h-3 w-3" />{rep.email}</div>
                                            )}
                                            {rep.phone && (
                                                <div className="flex items-center"><Phone className="mr-1.5 h-3 w-3" />{rep.phone}</div>
                                            )}
                                        </div>
                                    </td>
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatDate(rep.createdAt)}</td>
                                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                        <button onClick={() => handleOpenModal(rep)} className="p-2 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-gray-100 transition-colors" title="Edit">
                                            <Pencil className="h-4 w-4" />
                                        </button>
                                        <button onClick={() => handleDelete(rep.id, rep.name)} className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
                    <div className="w-full max-w-md bg-white rounded-xl shadow-2xl p-6 transform transition-all">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label htmlFor="sr-name" className="block text-sm font-semibold text-gray-700">Full Name</label>
                            ...
                            <div>
                                <label htmlFor="sr-email" className="block text-sm font-semibold text-gray-700">Email Address</label>
                            ...
                            <div>
                                <label htmlFor="sr-phone" className="block text-sm font-semibold text-gray-700">Phone Number</label>
